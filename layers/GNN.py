import torch
from torch_geometric.nn import BatchNorm, GCNConv, LayerNorm, SAGEConv, Sequential, APPNP,GATConv,SAGEConv
...
class GNN(nn.Module):
    def __init__(self, layer_sizes, batchnorm_mm=0.99):
        ...
        for in_dim, out_dim in zip(layer_sizes[:-1], layer_sizes[1:]):
            # layers.append( (nn.Dropout(drop_rate), 'x -> x'), )
            layers.append((GATConv(in_dim, out_dim//8,heads=8), 'x, edge_index -> x'), )
            # layers.append((GCNConv(in_dim, out_dim), 'x, edge_index -> x'), )
